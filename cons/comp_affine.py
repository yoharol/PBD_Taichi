...
class CompDynAffine2D:
  ...
  def compute_deriv_sum(self):
    ...
    for i in range(self.n_vert):
      for j in range(self.n_controls):
        ...
        w = self.weights[i, j]
        self.sum_deriv[j,
                       0] += m * w * w * self.v_p_ref[i][0] * self.v_p_ref[i][0]
        self.sum_deriv[j,
                       1] += m * w * w * self.v_p_ref[i][1] * self.v_p_ref[i][1]
  ...
  def update_selected_cons(self, selected_cons, alpha=-1.0):
    if alpha < 0.0:
      alpha = self.alpha
    self.solve_selected_cons(selected_cons, alpha)

  @ti.kernel
  def solve_selected_cons(self, j: ti.i32, alpha: ti.f32):
    for kdx in range(3):
      c = ti.Vector([0.0, 0.0])
      delta_lambda = ti.Vector([0.0, 0.0])
      for i in range(self.n_vert):
      ...
      for k in range(2):
        delta_lambda[k] = -(c[k] + alpha * self.lambdaf[j, kdx * 2 + k]) / (
            self.sum_deriv[j, kdx] + alpha)
        self.lambdaf[j, kdx * 2 + k] += delta_lambda[k]

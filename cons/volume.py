...
class Volume:
  ...
  def __init__(self, mesh: TrianMesh, dt, alpha=0.0) -> None:
    self.n = mesh.n_face
    self.pos = mesh.v_p
    self.ref_pos = mesh.v_p_ref
    self.face_indices = mesh.f_i
  ...
  def solve_cons(self):
    ...
    for k in range(self.n):
      ...
      self.pos[i1] += self.invm[i1] * delta_lambda * d1
      self.pos[i2] += self.invm[i2] * delta_lambda * d2
      self.pos[i3] += self.invm[i3] * delta_lambda * d3

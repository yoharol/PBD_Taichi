...
def get_quad_affine(v_p: np.ndarray, v_p_ref: np.ndarray, weights: np.ndarray,
                    mass: np.ndarray, c_pos: np.ndarray, c_pos_ref: np.ndarray):
  n_vert = v_p.shape[0]
  n_control = weights.shape[1]
  ...
  for i in range(n_vert):
    for j in range(n_control):
      ...
      quad_vec = np.array([q[0], q[1], q[0] * q[0], q[1] * q[1], q[0] * q[1]])
...
def quad_blend_skinning(v_p: np.ndarray, v_p_ref: np.ndarray,
  ...
  for i in range(n_vert):
    for j in range(n_control):
      q = v_p_ref[i] - c_pos_ref[j]
      quad_vec = np.array([q[0], q[1], q[0] * q[0], q[1] * q[1], q[0] * q[1]])
      v_p_rig[i] += weights[i, j] * (c_pos[j] + D[j] @ quad_vec)
  return v_p_rig
...
while window.running:
  ...
  for sub in range(substep):
    ...
    D, R, S = get_quad_affine(mesh.v_p.to_numpy(), mesh.v_p_ref.to_numpy(),
    ...
    for j in range(lbs.n_points):
      ...
      D[j, :, :2] = rigged_R[j]
      # D[j, :, :2] = rigged_R[j]
    quad_pos = quad_blend_skinning(mesh.v_p.to_numpy(), mesh.v_p_ref.to_numpy(),
                                   points.v_weights.to_numpy(),
                                   points.c_p.to_numpy(),
                                   points.c_p_ref.to_numpy(), D)
    lbs.v_p_rig.from_numpy(quad_pos)
    for j in range(lbs.n_points):
      shape.update_selected_cons(j)
